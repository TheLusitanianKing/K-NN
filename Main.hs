...
main = do
    ...
    let k = if length args == 1
            then fromMaybe defaultK $ readMaybe (head args)
            else defaultK

    -- retrieve CSV files
    learningCSV <- T.IO.readFile "data/football/learning.csv"
    classificationInputCSV <-
        T.IO.readFile "data/football/classification/input.csv"
    regressionInputCSV <-
        T.IO.readFile "data/football/regression/input-stadium.csv"
    regressionInputCSV' <-
        T.IO.readFile "data/football/regression/input-european.csv"

    -- k-nn classification
    knnClassification k 5 $
        learningCSV <> "\n" <> classificationInputCSV

    -- k-nn regression
    -- on stadium capacity
    knnRegression k 0 $ learningCSV <> "\n" <> regressionInputCSV
    -- on european trophies
    knnRegression k 1 $ learningCSV <> "\n" <> regressionInputCSV'
...
knnClassification k index s = do
    ...
    putStrLn $
        ...
        show (score predicted evaluationData) ++ "\n"
...
knnRegression k index s = do
    ...
    putStrLn $
        "R-Square (between 0 and 1): " ++
        show (rsquare predicted evaluationData) ++ "\n"