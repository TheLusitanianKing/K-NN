...
scaling i = i { objects = scaledObjects }
    where scaledObjects :: [Object]
          scaledObjects =
              map (\o -> o { features = zipWith minmaxScaling minmax (features o) })
                $ objects i
          -- minmax being for each column of variables the tuple (minimum value, maximum value)
          minmax = map (\vs -> (minimum vs, maximum vs)) . transpose . map features $ objects i
...
euclideanDistance ws o1 o2 = sqrt . sum $ weightedDists
    where weightedDists = zipWith (*) ws distances
          distances = zipWith (\v1 v2 -> (v2 - v1) ^ 2) (features o1) (features o2)
...
nearestNeighbours k e i = map (\(n, o, d) -> (n, fromJust . label $ o, d))
